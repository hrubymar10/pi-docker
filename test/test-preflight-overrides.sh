...
restore_local_state() {
  ...
  if [[ -n "${BACKUP_PI_NOTIFIER:-}" && -f "$BACKUP_PI_NOTIFIER" ]]; then
    mv "$BACKUP_PI_NOTIFIER" "$ROOT/config/pi-notifier"
  else
    rm -f "$ROOT/config/pi-notifier"
  fi
}
...
if [[ -f "$ROOT/config/pi-notifier" ]]; then
  BACKUP_PI_NOTIFIER="$TMP_ROOT/pi-notifier.bak"
  cp "$ROOT/config/pi-notifier" "$BACKUP_PI_NOTIFIER"
fi
rm -f "$ROOT/config/pi-notifier"

HOME_DIR="$TMP_ROOT/home/tester"
mkdir -p "$HOME_DIR/.ssh" "$HOME_DIR/.agents" "$HOME_DIR/projects/secrets" "$TMP_ROOT/pi-packages"
...
(
  ...
  export PI_PACKAGE_DIR="$TMP_ROOT/pi-packages"
  ...
  export GOPRIVATE=
  export GONOSUMDB=
  "$ROOT/bin/pi-docker-ctrl" start > "$TMP_ROOT/start.out" 2> "$TMP_ROOT/start.err"
)

if [[ -x "$ROOT/config/pi-notifier" ]]; then
  ok "pi-notifier auto-created from example"
else
  fail "pi-notifier was not auto-created"
fi

CONTEXT_FILES=("$TMPDIR_TEST"/pi-docker-context.*)
if [[ -f "${CONTEXT_FILES[0]}" ]] && grep -Rqs '/AGENTS.md:.*AGENTS.md:ro' "$TMPDIR_TEST"/pi-docker-context.* \
  && grep -Rqs '/CLAUDE.md:.*CLAUDE.md:ro' "$TMPDIR_TEST"/pi-docker-context.*; then
  ok "global AGENTS.md and CLAUDE.md overrides created"
else
  fail "missing AGENTS.md/CLAUDE.md override files"
fi

if ls "$TMPDIR_TEST"/pi-docker-ssh.* >/dev/null 2>&1 \
  && grep -Rqs '.ssh/known_hosts:.*known_hosts:ro' "$TMPDIR_TEST"/pi-docker-ssh.*; then
  ok "known_hosts override created"
else
  fail "missing known_hosts override"
fi

if ls "$TMPDIR_TEST"/pi-docker-agents.* >/dev/null 2>&1 \
  && grep -Rqs '/.agents:.*\.agents' "$TMPDIR_TEST"/pi-docker-agents.*; then
  ok "~/.agents override created"
else
  fail "missing ~/.agents override"
fi

if ls "$TMPDIR_TEST"/pi-docker-pi-package-dir.* >/dev/null 2>&1 \
  && grep -Rqs "$TMP_ROOT/pi-packages:$TMP_ROOT/pi-packages" "$TMPDIR_TEST"/pi-docker-pi-package-dir.*; then
  ok "PI_PACKAGE_DIR override created"
else
  fail "missing PI_PACKAGE_DIR override"
fi

if ls "$TMPDIR_TEST"/pi-docker-excludes.* >/dev/null 2>&1 \
  && grep -Rqs "/dev/null:$HOME_DIR/projects/.env:ro" "$TMPDIR_TEST"/pi-docker-excludes.* \
  && grep -Rqs "$HOME_DIR/projects/secrets:ro,size=0" "$TMPDIR_TEST"/pi-docker-excludes.*; then
...
if grep -q 'COMPOSE_PROJECT_NAME=pi-docker' "$LOG"; then
