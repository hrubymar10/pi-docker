...
SHELL := /bin/bash
export COMPOSE_PROJECT_NAME ?= pi-docker
...
exec: ## Start an interactive pi session inside the container
...
vscode-wrapper: ## Print path to the VS Code wrapper binary
	@printf '%s/bin/pi-docker-vscode-wrapper\n' "$$(pwd)"
